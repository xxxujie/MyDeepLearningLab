...
def _mha_shape_check(query: Tensor, key: Tensor, value: Tensor) -> bool:
    ...
    if query.dim() == 3:
        ...
        assert key.dim() == 3 and value.dim() == 3, (
            "对于 batched 3-D `query`，`key` 和 `value` 应为 3-D，"
            f"然而输入分别为 {key.dim()}-D 和 {value.dim()}-D"
        )
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(
        ...
        r"""进行 multi-head attention 的计算"""
        is_batched = _mha_shape_check(q_seq, k_seq, v_seq)
        # 如果 QKV 是 2 维的，增加一个 batch 维度
        ...
        # 这里为了简化模型，假定 QKV 的 embedding_dim 全部相等
        ...
        if input_embedding_dim != self.embedding_dim:
            raise ValueError(
                "输入的 `query`、`key` 和 `value` 的维度（embedding_dim）和 MHA 模型预设维度不匹配，"
        ...
        # 拆分最后一维 -> embedding_dim = head_count * head_dim，并交换 head_count 和 seq_dim 维度
        queries = (
            queries.contiguous()
...
class TokenEmbedding(nn.Embedding):
    """用来将 Token 转化为 embedding，其实就是封装了一下 nn.Embedding"""
...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, seq_len, embedding_dim):
        ...
        # pos 和 index 一个列向量，一个行向量，
        # 在计算时经过 python 广播，就得到了一个 (seq_len, embedding_dim) 的矩阵
        pos = torch.arange(0, seq_len)
        pos = pos.float().unsqueeze(1)
        index = torch.arange(0, embedding_dim)
        index = index.float().unsqueeze(0)
    ...
    def forward(self, input):
        ...
        if input_seq_len != self.seq_len or input_embedding_dim != self.embedding_dim:
            raise ValueError("输入数据的维度和 PositionEmbedding 模块维度不匹配")
...
class TransformerEmbedding(nn.Module):
    ...
    def __init__(self, vocab_size, seq_len, embedding_dim, dropout_prob):
        super().__init__()
...
class Encoder(nn.Module):
    ...
    def __init__(
        ...
        self.embedding = TransformerEmbedding(
            vocab_size, seq_len, embedding_dim, dropout_prob
        )
        self.encoder_layers = nn.ModuleList(
            EncoderLayer(embedding_dim, head_count, ffn_hidden_dim, dropout_prob)
...
class Decoder(nn.Module):
    ...
    def __init__(
        ...
        self.decoder_layers = nn.ModuleList(
            DecoderLayer(embedding_dim, head_count, ffn_hidden_dim, dropout_prob)
            for _ in range(layer_count)
        )
        # 最后的线性层
        self.full_connection = nn.Linear(embedding_dim, vocab_size)
...
class Transformer(nn.Module):
    ...
    def make_padding_mask(self, query: Tensor, key: Tensor, padding_index: int):
        ...
        query = query.ne(padding_index).unsqueeze(1).unsqueeze(3)
        query = query.repeat(1, 1, 1, key_seq_len)
        # query 扩展第 1 维和第 2 维，并将第 2 维重复到 query_seq_len 的数量
        key = key.ne(padding_index).unsqueeze(1).unsqueeze(2)
    ...
    def make_causal_mask(self, query: Tensor, key: Tensor):
        ...
        query_seq_len = query.shape[1]
        key_seq_len = key.shape[1]
        mask = torch.tril(torch.ones(query_seq_len, key_seq_len, dtype=bool))
    ...
    def forward(self, src: Tensor, tgt: Tensor):
        ...
        src_mask = self.make_padding_mask(src, src, self.padding_idx)

        # Decoder 处的自注意力模块，既有 padding mask，又有 causal mask
        _tgt_padding_mask = self.make_padding_mask(tgt, tgt, self.padding_idx)
        _tgt_causal_mask = self.make_causal_mask(tgt, tgt)
        tgt_mask = _tgt_padding_mask * _tgt_causal_mask  # 逐元素相乘

        # Encoder 和 Decoder 的交叉注意力模块，用到了 padding mask
        src_tgt_mask = self.make_padding_mask(src, tgt, self.padding_idx)
